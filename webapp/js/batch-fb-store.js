...
function loadLeftStore() {
    ...
    var batchid = $("body", parent.document).find("#tbody").find("input[type='checkbox']:checked").attr("id");
    var groupcode = $("#lgroup").val();
    $.ajax({
        url: "/batchFbStoresController/getBatchFbStoresList/"+batchid,    //请求的url地址
        dataType: "json",   //返回格式为json
...
function offWindows() {
    ...
    for (var i = 0; i < rightStoreData.length; i++) {
        storeIdArray[i] = rightStoreData.eq(i).attr("id");//门店ID
    }
    //loading层
    ...
    $.ajax({
        url: "/batchFbStoresController/saveBatchFbStores",
        type: "POST",
        data: {"batchId": batchId, "storeIdArray": storeIdArray.join(",")},
        dataType: "json",
        async: false,
        cache: false,
        success: function (data) {
...
function remove() {
    ...
    $.ajax({
        url: "/batchFbStoresController/removeBatchFbStores/" + batchId,
        type: "POST",
        data: {"storeIdArray": storeIdArray.join(",")},
        dataType: "json",
        async: false,
        cache: false,
        success: function (data) {
