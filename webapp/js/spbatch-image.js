/*浏览图片*/
$(function(){
	var id = getQueryString("batchid");
	$.ajax({
		url:pageContext+"/imageController/browseSpBatchImage",
		dataType:"json",   
		async:false,
		data:{"batchid":id},
		type:"post",  
...
function datadel(obj){
	...
	layer.confirm('确认删除吗？',function(index){
		$.ajax({
			url:pageContext+"/imageController/deleteSpBatchImage/"+imageId,
			dataType:"json",   
			async:true,
			type:"post",
			success:function(data){
...
function setTop(obj){
	var imageId = $(obj).parent().attr("id");
	$.ajax({
		url:pageContext+"/imageController/updateSpBatchImageTop/"+imageId,
		dataType:"json",   
		async:false,
		type:"post",
		success:function(data){
...
$(function(){
	...
	$(".picnumber").blur(function(){
		...
		if($(this).val() != '' && reg.test($(this).val()) && $(this).val() != currently){
			...
			$.ajax({
				url:pageContext+"/imageController/updateSpBatchImageNumber",
				dataType:"json",
				async:false,
